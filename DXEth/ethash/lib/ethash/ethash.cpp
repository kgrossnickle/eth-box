...
inline std::string to_hex(const ethash::hash512 & h)
{
...
inline std::string to_hex(const ethash::hash1024 & h)
{
    static const auto hex_chars = "0123456789abcdef";
    std::string str;
    str.reserve(sizeof(h) * 2);
    ...
    const epoch_context& context, const hash512& seed, lookup_fn lookup) noexcept
{
    ...
    uint32_t p;
    uint32_t firstp = 69420;
    for (uint32_t i = 0; i < num_dataset_accesses; ++i)
    {
        p = fnv1(i ^ seed_init, mix.word32s[i % num_words]) % index_limit;
        ...
        {
            firstp = p;
            std::cout << "first newdata hash : " << to_hex(newdata) << "\n";
        }

        if (i == num_dataset_accesses - 1)
        {
            std::cout << "last newdata hash : " << to_hex(newdata) << "\n";
        }
        for (size_t j = 0; j < num_words; ++j)
            mix.word32s[j] = fnv1(mix.word32s[j], newdata.word32s[j]);
    }
    std::cout << "num num_dataset_accesses, last p " << num_dataset_accesses << " " << p << "\n";
    std::cout << "first p: " << firstp << "\n";
    std::cout << "first mix which is 1024 mix : " << to_hex(mix) <<"\n";
    hash256 mix_hash;
...
{
    const hash512 seed = hash_seed(*header_hash, nonce);
    std::cout << "\n\n verif HASH 512 SEED: " << to_hex(seed) <<"\n";
